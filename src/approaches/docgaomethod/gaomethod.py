...
def stp_pos_flow(h, lv_act, t):
    H1t = 651+(42/19)*(t)  # H1中间包液位高度，时间函数，时间步为半秒，要除2
    ...
    dL = (pow(20, 0.5) * c2*A*pow((H1t+H2-H3), 0.5)*0.5) / (B * W)
    return dL
...
if __name__ == '__main__':
    ...
    for stage in range(5):
        ...
        for time in range(stopTimeSpan):
            if stage > 0:
                previousTime = ts[stage-1]
            else:
                previousTime = 0
            for sampleCount in range(sampleRate):
                lv_act += stp_pos_flow(hs[stage], lv_act, previousTime + time + (sampleCount / sampleRate))
